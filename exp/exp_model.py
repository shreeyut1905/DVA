...
class Exp_Model(object):
    ...
    def train(self,setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i,(batch_x,batch_y,x_mark,y_mark) in enumerate(train_loader):
                ...
                print("Epoch: {0}, Steps: {1} | MSE Loss: {2:.7f} KL Loss: {3:.7f} DSM Loss: {4:.7f} Overall Loss:{5:.7f}".format(epoch + 1, train_steps, mse, kl, dsm, all_loss))
    ...
    def test(self,setting):
        ...
        noisy = []
        input = []
        for i , (batch_x,batch_y,batch_x,mark,batch_y_mark) in enumerate(test_loader):
            batch_x = batch_x.float().to(self.device)
            batch_y  = batch_y[...,-self.args.target_dim:].float().to(self.device)
            batch_x_mark = batch_x_mark.float().to(self.device)
            noisy_out , out = self.pred_net(batch_x,batch_x_mark)
